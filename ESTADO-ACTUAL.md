...
- ✅ **Resuelto en la misma sesión:** las 6 facturas "de más" quedaron resueltas — 2 se debían a pagos que la usuaria registró manualmente en el Panel mientras se investigaba (folios 39588 y 46077, "asignado desde Pagos Excel"), y las 4 restantes se corrigieron con el propio recálculo del sistema durante la sesión. El resultado final coincide exacto con el sistema de facturación oficial: 2 facturas, $927.605.
- ✅ **CAUSA RAÍZ FINAL, la más importante — encontrada y corregida:** revisando el código fuente de `alertaCobranza` se descubrió `const ACTIVA = false; // INTERRUPTOR MAESTRO. false = solo simula.` — el interruptor maestro estaba apagado. Con esto en `false`, la función SIEMPRE calculaba todo bien pero nunca creaba la nota real en Kommo ni disparaba el WhatsApp real, solo lo simulaba en el log (`"simulado"`). Esto explica el reporte original de la usuaria de forma completa y definitiva, independiente de los otros 2 bugs corregidos hoy.
  - Encontrado también (evidencia en comentarios del propio código, fechados 12-08-2026): `ACTIVA` había estado en `true` más temprano ese mismo día, causando un bug de notas duplicadas por condición de carrera (lead 10771212 recibió 4 notas idénticas) — se corrigió con una transacción atómica de Firebase, pero `ACTIVA` quedó en `false` como medida de seguridad y nunca se reactivó.
  - **Verificado antes de activar (12-08-2026):** revisado el código completo de creación de la nota — usa `note_type: "common"` contra `/api/v4/leads/{id}/notes`, es una nota interna de Kommo, ningún endpoint de mensajería al cliente. Confirma el propio comentario del archivo: "no crea tareas, no envia mensajes al cliente, no modifica ningun campo, no borra nada."
  - **Respaldo tomado antes de modificar:** descargado el `index.js` de `alertaCobranza` completo (vía botón "Descargar archivo" de Cloud Run) antes de cualquier cambio, en el Mac de la usuaria.
  - **Cambio aplicado:** `ACTIVA = false` → `ACTIVA = true`, editado y desplegado directo desde la consola de Google Cloud Run (pestaña Fuente → Editar fuente → Guardar y volver a implementar) — esta función NO vive en el repo `fodor-deploy`, no hay git para versionarla (**deuda técnica anotada**: convendría en algún momento traer `alertaCobranza` a un repo con control de versiones).
  - **Validado en vivo con el caso real (12-08-2026):** disparado manualmente el webhook para el lead de EVENTOS E.M.LTDA (`#16613972`) → resultado `avisado(1/200)` (antes daba `simulado`). Confirmado en Kommo: nota real creada, ">>> DEUDA VENCIDA <<< $927.605 - EVENTOS E.M.LTDA, 2 factura(s) vencida(s), la mas antigua de 498 dias" — coincide EXACTO con el sistema de facturación oficial. Confirmado también en Firebase: entrada en `alertas_whatsapp_deborah` con `enviado:true` y el mismo monto — el WhatsApp se mandó.
- 🎉 **Estado final: el sistema de aviso de deuda vencida (Kommo + WhatsApp) quedó funcionando de punta a punta, validado con un caso real, sin pendientes.**
