...
function leerComoMatriz_(archivo) {
  ...
  try {
    var recurso = {
      title: 'TMP_' + archivo.getName(),
      mimeType: MimeType.GOOGLE_SHEETS
    };
    // Requiere el servicio avanzado "Drive API" activado.
    temp = Drive.Files.copy(recurso, archivo.getId(), { convert: true });
    var hoja = SpreadsheetApp.openById(temp.id).getSheets()[0];
